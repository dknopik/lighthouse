...
use lighthouse_network::rpc::methods::MAX_REQUEST_BLOBS;
...
impl<T: BeaconChainTypes> Worker<T> {
    ...
    // TODO(dknopik): read wether this is fine
    /// Handle a `BlobsByRange` request from the peer.
    pub fn handle_blobs_sidecars_by_range_request(
        self,
        executor: TaskExecutor,
        send_on_drop: SendOnDrop,
        peer_id: PeerId,
        request_id: PeerRequestId,
        mut req: BlobsSidecarsByRangeRequest,
    ) {
        debug!(self.log, "Received BlobsSidecarsByRange Request";
            "peer_id" => %peer_id,
            "count" => req.count,
            "start_slot" => req.start_slot,
        );
        ...
        // Should not send more than max request blocks
        if req.count > MAX_REQUEST_BLOBS {
            req.count = MAX_REQUEST_BLOBS;
        }
                ...
                for root in block_roots {
                    match self.chain.store.get_blobs(&root) {
                        Ok(Some(blob)) => {
                            ...
                            self.send_network_message(NetworkMessage::SendResponse {
                                peer_id,
                                response: Response::BlobsByRange(Some(Arc::new(
                                    VariableList::new(vec![blob.message]).unwrap(),
                                ))),
                                id: request_id,
                            });
                        }
